...
def stats_customer_orders():
  ...
  It's aggregating data across three tables to show who are most valuable customers based 
  on their historical spending and order frequency.
  '''
...
def stats_product_orders():
  ...
  Designed to show which items are driving the revenue and 
  how fast they are moving off the shelves.
  '''
  with get_conn() as conn, conn.cursor() as cur:
      cur.execute("""
          SELECT
...
def stats_top_products(n: int = 10):
  ...
  A more focused version of the product performance report,
  highlighting the top-selling items to help with inventory and marketing decisions.
  '''
  if n <= 0 or n > 10:
...
def stats_recent_sales(n: int = 30):
  '''
  Recent Sales Trends: 
  Focuses on the most recent sales data to identify emerging trends and seasonal patterns.
  '''
  if n <= 0 or n > 90:
